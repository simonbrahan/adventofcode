...
(defn all-minute-sleep-frequencies [guard-id minutes-asleep]
  (map (fn [minute-count] (cons guard-id minute-count)) (into [] (frequencies minutes-asleep))))

(let [minutes-asleep-by-guard (-> input-lines prepare-notes add-guard-ids get-all-sleep-minutes)
      minute-records (mapcat #(apply all-minute-sleep-frequencies %) (into [] minutes-asleep-by-guard))
      [guard-id minute _] (apply max-key last minute-records)]
  (prn (* guard-id minute)))
